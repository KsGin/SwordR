...
namespace SwordR
{
    void GraphicsPipeline::create(Device* device, GraphicsPipelineCreateInfo info)
    {
        this->device = device;
        this->info = info;
        createPipeline();
    ...
    void GraphicsPipeline::createPipeline() {
    	...
    	if (info.type == ModelRenderPipeline)
        {
        ...
        {
            auto bindingDescription = ParticleSystem::Vertex::getBindingDescription();
            auto attributeDescriptions = ParticleSystem::Vertex::getAttributeDescriptions();
            vertexInputInfo.pVertexBindingDescriptions = &bindingDescription;
            vertexInputInfo.vertexAttributeDescriptionCount = static_cast<uint32_t>(attributeDescriptions.size());;
            vertexInputInfo.pVertexAttributeDescriptions = attributeDescriptions.data(); // Optional
        }
        ...
        if (info.type == ModelRenderPipeline)
        {
            ...
            VkDescriptorSetLayoutBinding deviceUBOLayoutBinding{};
            deviceUBOLayoutBinding.binding = 2;
            deviceUBOLayoutBinding.descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
            deviceUBOLayoutBinding.descriptorCount = 1;
            deviceUBOLayoutBinding.stageFlags = VK_SHADER_STAGE_VERTEX_BIT;
            deviceUBOLayoutBinding.pImmutableSamplers = nullptr; // Optional
            ...
            std::array<VkDescriptorSetLayoutBinding, 4> bindings = { cameraUBOLayoutBinding, modelUBOLayoutBinding, deviceUBOLayoutBinding, samplerLayoutBinding };
        ...
        {
            VkDescriptorSetLayoutBinding deviceUBOLayoutBinding{};
            deviceUBOLayoutBinding.binding = 1;
            deviceUBOLayoutBinding.descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
            deviceUBOLayoutBinding.descriptorCount = 1;
            deviceUBOLayoutBinding.stageFlags = VK_SHADER_STAGE_VERTEX_BIT;
            deviceUBOLayoutBinding.pImmutableSamplers = nullptr; // Optional

            std::array<VkDescriptorSetLayoutBinding, 2> bindings = { cameraUBOLayoutBinding, deviceUBOLayoutBinding };
        ...
        auto shaderName = graphicsNameList[static_cast<int>(info.type)];
        ...
        std::cout << "Compile Vertex Shader : Execute " << vsShaderCompileCommand << std::endl;
        system(vsShaderCompileCommand.c_str());
        std::cout << "Compile Fragment Shader : Execute " << fsShaderCompileCommand << std::endl;
    ...
    void GraphicsPipeline::createDescriptorSets() {
        ...
        for (size_t i = 0; i < device->MAX_FRAMES_IN_FLIGHT; i++) {
	        ...
	        else
	        {
                ...
                descriptorWrites[1].descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
                descriptorWrites[1].descriptorCount = 1;
                descriptorWrites[1].pBufferInfo = &deviceUBOInfo;
                vkUpdateDescriptorSets(device->logicalDevice, static_cast<uint32_t>(descriptorWrites.size()), descriptorWrites.data(), 0, nullptr);
	        }
        }
    }
